(* generated by: awso-codegen generate-all --botocore-data vendor/botocore/botocore/data -o aws --runtime-dir lib/runtime/awso --cli-dir awso-cli *)
open Awso_route53domains.Values
val accept_domain_transfer_from_another_aws_account :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      AcceptDomainTransferFromAnotherAwsAccountRequest.t ->
        (AcceptDomainTransferFromAnotherAwsAccountResponse.t,
          AcceptDomainTransferFromAnotherAwsAccountResponse.error) Result.t
          Async.Deferred.t
val associate_delegation_signer_to_domain :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      AssociateDelegationSignerToDomainRequest.t ->
        (AssociateDelegationSignerToDomainResponse.t,
          AssociateDelegationSignerToDomainResponse.error) Result.t
          Async.Deferred.t
val cancel_domain_transfer_to_another_aws_account :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      CancelDomainTransferToAnotherAwsAccountRequest.t ->
        (CancelDomainTransferToAnotherAwsAccountResponse.t,
          CancelDomainTransferToAnotherAwsAccountResponse.error) Result.t
          Async.Deferred.t
val check_domain_availability :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      CheckDomainAvailabilityRequest.t ->
        (CheckDomainAvailabilityResponse.t,
          CheckDomainAvailabilityResponse.error) Result.t Async.Deferred.t
val check_domain_transferability :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      CheckDomainTransferabilityRequest.t ->
        (CheckDomainTransferabilityResponse.t,
          CheckDomainTransferabilityResponse.error) Result.t Async.Deferred.t
val delete_domain :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DeleteDomainRequest.t ->
        (DeleteDomainResponse.t, DeleteDomainResponse.error) Result.t
          Async.Deferred.t
val delete_tags_for_domain :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DeleteTagsForDomainRequest.t ->
        (DeleteTagsForDomainResponse.t, DeleteTagsForDomainResponse.error)
          Result.t Async.Deferred.t
val disable_domain_auto_renew :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DisableDomainAutoRenewRequest.t ->
        (DisableDomainAutoRenewResponse.t,
          DisableDomainAutoRenewResponse.error) Result.t Async.Deferred.t
val disable_domain_transfer_lock :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DisableDomainTransferLockRequest.t ->
        (DisableDomainTransferLockResponse.t,
          DisableDomainTransferLockResponse.error) Result.t Async.Deferred.t
val disassociate_delegation_signer_from_domain :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DisassociateDelegationSignerFromDomainRequest.t ->
        (DisassociateDelegationSignerFromDomainResponse.t,
          DisassociateDelegationSignerFromDomainResponse.error) Result.t
          Async.Deferred.t
val enable_domain_auto_renew :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      EnableDomainAutoRenewRequest.t ->
        (EnableDomainAutoRenewResponse.t,
          EnableDomainAutoRenewResponse.error) Result.t Async.Deferred.t
val enable_domain_transfer_lock :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      EnableDomainTransferLockRequest.t ->
        (EnableDomainTransferLockResponse.t,
          EnableDomainTransferLockResponse.error) Result.t Async.Deferred.t
val get_contact_reachability_status :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      GetContactReachabilityStatusRequest.t ->
        (GetContactReachabilityStatusResponse.t,
          GetContactReachabilityStatusResponse.error) Result.t
          Async.Deferred.t
val get_domain_detail :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      GetDomainDetailRequest.t ->
        (GetDomainDetailResponse.t, GetDomainDetailResponse.error) Result.t
          Async.Deferred.t
val get_domain_suggestions :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      GetDomainSuggestionsRequest.t ->
        (GetDomainSuggestionsResponse.t, GetDomainSuggestionsResponse.error)
          Result.t Async.Deferred.t
val get_operation_detail :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      GetOperationDetailRequest.t ->
        (GetOperationDetailResponse.t, GetOperationDetailResponse.error)
          Result.t Async.Deferred.t
val list_domains :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      ListDomainsRequest.t ->
        (ListDomainsResponse.t, ListDomainsResponse.error) Result.t
          Async.Deferred.t
val list_operations :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      ListOperationsRequest.t ->
        (ListOperationsResponse.t, ListOperationsResponse.error) Result.t
          Async.Deferred.t
val list_prices :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      ListPricesRequest.t ->
        (ListPricesResponse.t, ListPricesResponse.error) Result.t
          Async.Deferred.t
val list_tags_for_domain :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      ListTagsForDomainRequest.t ->
        (ListTagsForDomainResponse.t, ListTagsForDomainResponse.error)
          Result.t Async.Deferred.t
val push_domain :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      PushDomainRequest.t -> (unit, unit) Result.t Async.Deferred.t
val register_domain :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      RegisterDomainRequest.t ->
        (RegisterDomainResponse.t, RegisterDomainResponse.error) Result.t
          Async.Deferred.t
val reject_domain_transfer_from_another_aws_account :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      RejectDomainTransferFromAnotherAwsAccountRequest.t ->
        (RejectDomainTransferFromAnotherAwsAccountResponse.t,
          RejectDomainTransferFromAnotherAwsAccountResponse.error) Result.t
          Async.Deferred.t
val renew_domain :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      RenewDomainRequest.t ->
        (RenewDomainResponse.t, RenewDomainResponse.error) Result.t
          Async.Deferred.t
val resend_contact_reachability_email :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      ResendContactReachabilityEmailRequest.t ->
        (ResendContactReachabilityEmailResponse.t,
          ResendContactReachabilityEmailResponse.error) Result.t
          Async.Deferred.t
val resend_operation_authorization :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      ResendOperationAuthorizationRequest.t ->
        (unit, unit) Result.t Async.Deferred.t
val retrieve_domain_auth_code :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      RetrieveDomainAuthCodeRequest.t ->
        (RetrieveDomainAuthCodeResponse.t,
          RetrieveDomainAuthCodeResponse.error) Result.t Async.Deferred.t
val transfer_domain :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      TransferDomainRequest.t ->
        (TransferDomainResponse.t, TransferDomainResponse.error) Result.t
          Async.Deferred.t
val transfer_domain_to_another_aws_account :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      TransferDomainToAnotherAwsAccountRequest.t ->
        (TransferDomainToAnotherAwsAccountResponse.t,
          TransferDomainToAnotherAwsAccountResponse.error) Result.t
          Async.Deferred.t
val update_domain_contact :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      UpdateDomainContactRequest.t ->
        (UpdateDomainContactResponse.t, UpdateDomainContactResponse.error)
          Result.t Async.Deferred.t
val update_domain_contact_privacy :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      UpdateDomainContactPrivacyRequest.t ->
        (UpdateDomainContactPrivacyResponse.t,
          UpdateDomainContactPrivacyResponse.error) Result.t Async.Deferred.t
val update_domain_nameservers :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      UpdateDomainNameserversRequest.t ->
        (UpdateDomainNameserversResponse.t,
          UpdateDomainNameserversResponse.error) Result.t Async.Deferred.t
val update_tags_for_domain :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      UpdateTagsForDomainRequest.t ->
        (UpdateTagsForDomainResponse.t, UpdateTagsForDomainResponse.error)
          Result.t Async.Deferred.t
val view_billing :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      ViewBillingRequest.t ->
        (ViewBillingResponse.t, ViewBillingResponse.error) Result.t
          Async.Deferred.t