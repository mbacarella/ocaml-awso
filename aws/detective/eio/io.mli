(* generated by: awso-codegen generate-all --botocore-data vendor/botocore/botocore/data -o aws --runtime-dir lib/runtime/awso --cli-dir awso-cli *)
open Awso_detective.Values
val accept_invitation :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t -> AcceptInvitationRequest.t -> (unit, unit) Result.t
val batch_get_graph_member_datasources :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t ->
      BatchGetGraphMemberDatasourcesRequest.t ->
        (BatchGetGraphMemberDatasourcesResponse.t,
          BatchGetGraphMemberDatasourcesResponse.error) Result.t
val batch_get_membership_datasources :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t ->
      BatchGetMembershipDatasourcesRequest.t ->
        (BatchGetMembershipDatasourcesResponse.t,
          BatchGetMembershipDatasourcesResponse.error) Result.t
val create_graph :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t ->
      CreateGraphRequest.t ->
        (CreateGraphResponse.t, CreateGraphResponse.error) Result.t
val create_members :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t ->
      CreateMembersRequest.t ->
        (CreateMembersResponse.t, CreateMembersResponse.error) Result.t
val delete_graph :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t -> DeleteGraphRequest.t -> (unit, unit) Result.t
val delete_members :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t ->
      DeleteMembersRequest.t ->
        (DeleteMembersResponse.t, DeleteMembersResponse.error) Result.t
val describe_organization_configuration :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t ->
      DescribeOrganizationConfigurationRequest.t ->
        (DescribeOrganizationConfigurationResponse.t,
          DescribeOrganizationConfigurationResponse.error) Result.t
val disable_organization_admin_account :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t -> unit -> (unit, unit) Result.t
val disassociate_membership :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t ->
      DisassociateMembershipRequest.t -> (unit, unit) Result.t
val enable_organization_admin_account :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t ->
      EnableOrganizationAdminAccountRequest.t -> (unit, unit) Result.t
val get_investigation :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t ->
      GetInvestigationRequest.t ->
        (GetInvestigationResponse.t, GetInvestigationResponse.error) Result.t
val get_members :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t ->
      GetMembersRequest.t ->
        (GetMembersResponse.t, GetMembersResponse.error) Result.t
val list_datasource_packages :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t ->
      ListDatasourcePackagesRequest.t ->
        (ListDatasourcePackagesResponse.t,
          ListDatasourcePackagesResponse.error) Result.t
val list_graphs :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t ->
      ListGraphsRequest.t ->
        (ListGraphsResponse.t, ListGraphsResponse.error) Result.t
val list_indicators :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t ->
      ListIndicatorsRequest.t ->
        (ListIndicatorsResponse.t, ListIndicatorsResponse.error) Result.t
val list_investigations :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t ->
      ListInvestigationsRequest.t ->
        (ListInvestigationsResponse.t, ListInvestigationsResponse.error)
          Result.t
val list_invitations :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t ->
      ListInvitationsRequest.t ->
        (ListInvitationsResponse.t, ListInvitationsResponse.error) Result.t
val list_members :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t ->
      ListMembersRequest.t ->
        (ListMembersResponse.t, ListMembersResponse.error) Result.t
val list_organization_admin_accounts :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t ->
      ListOrganizationAdminAccountsRequest.t ->
        (ListOrganizationAdminAccountsResponse.t,
          ListOrganizationAdminAccountsResponse.error) Result.t
val list_tags_for_resource :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t ->
      ListTagsForResourceRequest.t ->
        (ListTagsForResourceResponse.t, ListTagsForResourceResponse.error)
          Result.t
val reject_invitation :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t -> RejectInvitationRequest.t -> (unit, unit) Result.t
val start_investigation :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t ->
      StartInvestigationRequest.t ->
        (StartInvestigationResponse.t, StartInvestigationResponse.error)
          Result.t
val start_monitoring_member :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t ->
      StartMonitoringMemberRequest.t -> (unit, unit) Result.t
val tag_resource :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t ->
      TagResourceRequest.t ->
        (TagResourceResponse.t, TagResourceResponse.error) Result.t
val untag_resource :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t ->
      UntagResourceRequest.t ->
        (UntagResourceResponse.t, UntagResourceResponse.error) Result.t
val update_datasource_packages :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t ->
      UpdateDatasourcePackagesRequest.t -> (unit, unit) Result.t
val update_investigation_state :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t ->
      UpdateInvestigationStateRequest.t -> (unit, unit) Result.t
val update_organization_configuration :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t ->
      UpdateOrganizationConfigurationRequest.t -> (unit, unit) Result.t