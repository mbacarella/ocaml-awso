(* generated by: awso-codegen generate-all --botocore-data vendor/botocore/botocore/data -o aws --runtime-dir lib/runtime/awso --cli-dir awso-cli *)
open Awso_elasticache.Values
val add_tags_to_resource :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      AddTagsToResourceMessage.t ->
        (TagListMessage.t, TagListMessage.error) Result.t Lwt.t
val authorize_cache_security_group_ingress :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      AuthorizeCacheSecurityGroupIngressMessage.t ->
        (AuthorizeCacheSecurityGroupIngressResult.t,
          AuthorizeCacheSecurityGroupIngressResult.error) Result.t Lwt.t
val batch_apply_update_action :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      BatchApplyUpdateActionMessage.t ->
        (UpdateActionResultsMessage.t, UpdateActionResultsMessage.error)
          Result.t Lwt.t
val batch_stop_update_action :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      BatchStopUpdateActionMessage.t ->
        (UpdateActionResultsMessage.t, UpdateActionResultsMessage.error)
          Result.t Lwt.t
val complete_migration :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      CompleteMigrationMessage.t ->
        (CompleteMigrationResponse.t, CompleteMigrationResponse.error)
          Result.t Lwt.t
val copy_serverless_cache_snapshot :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      CopyServerlessCacheSnapshotRequest.t ->
        (CopyServerlessCacheSnapshotResponse.t,
          CopyServerlessCacheSnapshotResponse.error) Result.t Lwt.t
val copy_snapshot :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      CopySnapshotMessage.t ->
        (CopySnapshotResult.t, CopySnapshotResult.error) Result.t Lwt.t
val create_cache_cluster :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      CreateCacheClusterMessage.t ->
        (CreateCacheClusterResult.t, CreateCacheClusterResult.error) Result.t
          Lwt.t
val create_cache_parameter_group :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      CreateCacheParameterGroupMessage.t ->
        (CreateCacheParameterGroupResult.t,
          CreateCacheParameterGroupResult.error) Result.t Lwt.t
val create_cache_security_group :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      CreateCacheSecurityGroupMessage.t ->
        (CreateCacheSecurityGroupResult.t,
          CreateCacheSecurityGroupResult.error) Result.t Lwt.t
val create_cache_subnet_group :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      CreateCacheSubnetGroupMessage.t ->
        (CreateCacheSubnetGroupResult.t, CreateCacheSubnetGroupResult.error)
          Result.t Lwt.t
val create_global_replication_group :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      CreateGlobalReplicationGroupMessage.t ->
        (CreateGlobalReplicationGroupResult.t,
          CreateGlobalReplicationGroupResult.error) Result.t Lwt.t
val create_replication_group :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      CreateReplicationGroupMessage.t ->
        (CreateReplicationGroupResult.t, CreateReplicationGroupResult.error)
          Result.t Lwt.t
val create_serverless_cache :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      CreateServerlessCacheRequest.t ->
        (CreateServerlessCacheResponse.t,
          CreateServerlessCacheResponse.error) Result.t Lwt.t
val create_serverless_cache_snapshot :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      CreateServerlessCacheSnapshotRequest.t ->
        (CreateServerlessCacheSnapshotResponse.t,
          CreateServerlessCacheSnapshotResponse.error) Result.t Lwt.t
val create_snapshot :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      CreateSnapshotMessage.t ->
        (CreateSnapshotResult.t, CreateSnapshotResult.error) Result.t Lwt.t
val create_user :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      CreateUserMessage.t -> (User.t, User.error) Result.t Lwt.t
val create_user_group :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      CreateUserGroupMessage.t ->
        (UserGroup.t, UserGroup.error) Result.t Lwt.t
val decrease_node_groups_in_global_replication_group :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DecreaseNodeGroupsInGlobalReplicationGroupMessage.t ->
        (DecreaseNodeGroupsInGlobalReplicationGroupResult.t,
          DecreaseNodeGroupsInGlobalReplicationGroupResult.error) Result.t
          Lwt.t
val decrease_replica_count :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DecreaseReplicaCountMessage.t ->
        (DecreaseReplicaCountResult.t, DecreaseReplicaCountResult.error)
          Result.t Lwt.t
val delete_cache_cluster :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DeleteCacheClusterMessage.t ->
        (DeleteCacheClusterResult.t, DeleteCacheClusterResult.error) Result.t
          Lwt.t
val delete_cache_parameter_group :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DeleteCacheParameterGroupMessage.t -> (unit, unit) Result.t Lwt.t
val delete_cache_security_group :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DeleteCacheSecurityGroupMessage.t -> (unit, unit) Result.t Lwt.t
val delete_cache_subnet_group :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DeleteCacheSubnetGroupMessage.t -> (unit, unit) Result.t Lwt.t
val delete_global_replication_group :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DeleteGlobalReplicationGroupMessage.t ->
        (DeleteGlobalReplicationGroupResult.t,
          DeleteGlobalReplicationGroupResult.error) Result.t Lwt.t
val delete_replication_group :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DeleteReplicationGroupMessage.t ->
        (DeleteReplicationGroupResult.t, DeleteReplicationGroupResult.error)
          Result.t Lwt.t
val delete_serverless_cache :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DeleteServerlessCacheRequest.t ->
        (DeleteServerlessCacheResponse.t,
          DeleteServerlessCacheResponse.error) Result.t Lwt.t
val delete_serverless_cache_snapshot :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DeleteServerlessCacheSnapshotRequest.t ->
        (DeleteServerlessCacheSnapshotResponse.t,
          DeleteServerlessCacheSnapshotResponse.error) Result.t Lwt.t
val delete_snapshot :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DeleteSnapshotMessage.t ->
        (DeleteSnapshotResult.t, DeleteSnapshotResult.error) Result.t Lwt.t
val delete_user :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DeleteUserMessage.t -> (User.t, User.error) Result.t Lwt.t
val delete_user_group :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DeleteUserGroupMessage.t ->
        (UserGroup.t, UserGroup.error) Result.t Lwt.t
val describe_cache_clusters :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DescribeCacheClustersMessage.t ->
        (CacheClusterMessage.t, CacheClusterMessage.error) Result.t Lwt.t
val describe_cache_engine_versions :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DescribeCacheEngineVersionsMessage.t ->
        (CacheEngineVersionMessage.t, CacheEngineVersionMessage.error)
          Result.t Lwt.t
val describe_cache_parameter_groups :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DescribeCacheParameterGroupsMessage.t ->
        (CacheParameterGroupsMessage.t, CacheParameterGroupsMessage.error)
          Result.t Lwt.t
val describe_cache_parameters :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DescribeCacheParametersMessage.t ->
        (CacheParameterGroupDetails.t, CacheParameterGroupDetails.error)
          Result.t Lwt.t
val describe_cache_security_groups :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DescribeCacheSecurityGroupsMessage.t ->
        (CacheSecurityGroupMessage.t, CacheSecurityGroupMessage.error)
          Result.t Lwt.t
val describe_cache_subnet_groups :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DescribeCacheSubnetGroupsMessage.t ->
        (CacheSubnetGroupMessage.t, CacheSubnetGroupMessage.error) Result.t
          Lwt.t
val describe_engine_default_parameters :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DescribeEngineDefaultParametersMessage.t ->
        (DescribeEngineDefaultParametersResult.t,
          DescribeEngineDefaultParametersResult.error) Result.t Lwt.t
val describe_events :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DescribeEventsMessage.t ->
        (EventsMessage.t, EventsMessage.error) Result.t Lwt.t
val describe_global_replication_groups :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DescribeGlobalReplicationGroupsMessage.t ->
        (DescribeGlobalReplicationGroupsResult.t,
          DescribeGlobalReplicationGroupsResult.error) Result.t Lwt.t
val describe_replication_groups :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DescribeReplicationGroupsMessage.t ->
        (ReplicationGroupMessage.t, ReplicationGroupMessage.error) Result.t
          Lwt.t
val describe_reserved_cache_nodes :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DescribeReservedCacheNodesMessage.t ->
        (ReservedCacheNodeMessage.t, ReservedCacheNodeMessage.error) Result.t
          Lwt.t
val describe_reserved_cache_nodes_offerings :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DescribeReservedCacheNodesOfferingsMessage.t ->
        (ReservedCacheNodesOfferingMessage.t,
          ReservedCacheNodesOfferingMessage.error) Result.t Lwt.t
val describe_serverless_cache_snapshots :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DescribeServerlessCacheSnapshotsRequest.t ->
        (DescribeServerlessCacheSnapshotsResponse.t,
          DescribeServerlessCacheSnapshotsResponse.error) Result.t Lwt.t
val describe_serverless_caches :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DescribeServerlessCachesRequest.t ->
        (DescribeServerlessCachesResponse.t,
          DescribeServerlessCachesResponse.error) Result.t Lwt.t
val describe_service_updates :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DescribeServiceUpdatesMessage.t ->
        (ServiceUpdatesMessage.t, ServiceUpdatesMessage.error) Result.t Lwt.t
val describe_snapshots :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DescribeSnapshotsMessage.t ->
        (DescribeSnapshotsListMessage.t, DescribeSnapshotsListMessage.error)
          Result.t Lwt.t
val describe_update_actions :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DescribeUpdateActionsMessage.t ->
        (UpdateActionsMessage.t, UpdateActionsMessage.error) Result.t Lwt.t
val describe_user_groups :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DescribeUserGroupsMessage.t ->
        (DescribeUserGroupsResult.t, DescribeUserGroupsResult.error) Result.t
          Lwt.t
val describe_users :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DescribeUsersMessage.t ->
        (DescribeUsersResult.t, DescribeUsersResult.error) Result.t Lwt.t
val disassociate_global_replication_group :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DisassociateGlobalReplicationGroupMessage.t ->
        (DisassociateGlobalReplicationGroupResult.t,
          DisassociateGlobalReplicationGroupResult.error) Result.t Lwt.t
val export_serverless_cache_snapshot :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      ExportServerlessCacheSnapshotRequest.t ->
        (ExportServerlessCacheSnapshotResponse.t,
          ExportServerlessCacheSnapshotResponse.error) Result.t Lwt.t
val failover_global_replication_group :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      FailoverGlobalReplicationGroupMessage.t ->
        (FailoverGlobalReplicationGroupResult.t,
          FailoverGlobalReplicationGroupResult.error) Result.t Lwt.t
val increase_node_groups_in_global_replication_group :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      IncreaseNodeGroupsInGlobalReplicationGroupMessage.t ->
        (IncreaseNodeGroupsInGlobalReplicationGroupResult.t,
          IncreaseNodeGroupsInGlobalReplicationGroupResult.error) Result.t
          Lwt.t
val increase_replica_count :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      IncreaseReplicaCountMessage.t ->
        (IncreaseReplicaCountResult.t, IncreaseReplicaCountResult.error)
          Result.t Lwt.t
val list_allowed_node_type_modifications :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      ListAllowedNodeTypeModificationsMessage.t ->
        (AllowedNodeTypeModificationsMessage.t,
          AllowedNodeTypeModificationsMessage.error) Result.t Lwt.t
val list_tags_for_resource :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      ListTagsForResourceMessage.t ->
        (TagListMessage.t, TagListMessage.error) Result.t Lwt.t
val modify_cache_cluster :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      ModifyCacheClusterMessage.t ->
        (ModifyCacheClusterResult.t, ModifyCacheClusterResult.error) Result.t
          Lwt.t
val modify_cache_parameter_group :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      ModifyCacheParameterGroupMessage.t ->
        (CacheParameterGroupNameMessage.t,
          CacheParameterGroupNameMessage.error) Result.t Lwt.t
val modify_cache_subnet_group :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      ModifyCacheSubnetGroupMessage.t ->
        (ModifyCacheSubnetGroupResult.t, ModifyCacheSubnetGroupResult.error)
          Result.t Lwt.t
val modify_global_replication_group :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      ModifyGlobalReplicationGroupMessage.t ->
        (ModifyGlobalReplicationGroupResult.t,
          ModifyGlobalReplicationGroupResult.error) Result.t Lwt.t
val modify_replication_group :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      ModifyReplicationGroupMessage.t ->
        (ModifyReplicationGroupResult.t, ModifyReplicationGroupResult.error)
          Result.t Lwt.t
val modify_replication_group_shard_configuration :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      ModifyReplicationGroupShardConfigurationMessage.t ->
        (ModifyReplicationGroupShardConfigurationResult.t,
          ModifyReplicationGroupShardConfigurationResult.error) Result.t
          Lwt.t
val modify_serverless_cache :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      ModifyServerlessCacheRequest.t ->
        (ModifyServerlessCacheResponse.t,
          ModifyServerlessCacheResponse.error) Result.t Lwt.t
val modify_user :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      ModifyUserMessage.t -> (User.t, User.error) Result.t Lwt.t
val modify_user_group :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      ModifyUserGroupMessage.t ->
        (UserGroup.t, UserGroup.error) Result.t Lwt.t
val purchase_reserved_cache_nodes_offering :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      PurchaseReservedCacheNodesOfferingMessage.t ->
        (PurchaseReservedCacheNodesOfferingResult.t,
          PurchaseReservedCacheNodesOfferingResult.error) Result.t Lwt.t
val rebalance_slots_in_global_replication_group :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      RebalanceSlotsInGlobalReplicationGroupMessage.t ->
        (RebalanceSlotsInGlobalReplicationGroupResult.t,
          RebalanceSlotsInGlobalReplicationGroupResult.error) Result.t Lwt.t
val reboot_cache_cluster :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      RebootCacheClusterMessage.t ->
        (RebootCacheClusterResult.t, RebootCacheClusterResult.error) Result.t
          Lwt.t
val remove_tags_from_resource :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      RemoveTagsFromResourceMessage.t ->
        (TagListMessage.t, TagListMessage.error) Result.t Lwt.t
val reset_cache_parameter_group :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      ResetCacheParameterGroupMessage.t ->
        (CacheParameterGroupNameMessage.t,
          CacheParameterGroupNameMessage.error) Result.t Lwt.t
val revoke_cache_security_group_ingress :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      RevokeCacheSecurityGroupIngressMessage.t ->
        (RevokeCacheSecurityGroupIngressResult.t,
          RevokeCacheSecurityGroupIngressResult.error) Result.t Lwt.t
val start_migration :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      StartMigrationMessage.t ->
        (StartMigrationResponse.t, StartMigrationResponse.error) Result.t
          Lwt.t
val test_failover :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      TestFailoverMessage.t ->
        (TestFailoverResult.t, TestFailoverResult.error) Result.t Lwt.t
val test_migration :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      TestMigrationMessage.t ->
        (TestMigrationResponse.t, TestMigrationResponse.error) Result.t Lwt.t