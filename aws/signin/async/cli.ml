...
let create_o_auth2_token =
  Command.async ~summary:""
    ([%map_open.Command
       ...
       and tokenInput =
         flag "token-input" (required json_arg)
           ~doc:"JSON CreateOAuth2TokenRequestBody" in
       fun () ->
         call ?endpoint_url ?profile:cli_profile ?region:cli_region
           Io.create_o_auth2_token
           (Values.CreateOAuth2TokenRequest.make
              ~tokenInput:(Values.CreateOAuth2TokenRequestBody.of_json
                             tokenInput) ())
           (Some Values.CreateOAuth2TokenResponse.to_json)
           (Some Values.CreateOAuth2TokenResponse.error_to_json)])
let main =
  Command.group
    ~summary:((Awso.Service.to_string Values.service) ^ " commands")
    [("create-o-auth2-token", create_o_auth2_token)]