(* generated by: awso-codegen generate-all --botocore-data vendor/botocore/botocore/data -o aws --runtime-dir lib/runtime/awso --cli-dir awso-cli *)
open Awso_application_autoscaling.Values
val delete_scaling_policy :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t ->
      DeleteScalingPolicyRequest.t ->
        (DeleteScalingPolicyResponse.t, DeleteScalingPolicyResponse.error)
          Result.t
val delete_scheduled_action :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t ->
      DeleteScheduledActionRequest.t ->
        (DeleteScheduledActionResponse.t,
          DeleteScheduledActionResponse.error) Result.t
val deregister_scalable_target :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t ->
      DeregisterScalableTargetRequest.t ->
        (DeregisterScalableTargetResponse.t,
          DeregisterScalableTargetResponse.error) Result.t
val describe_scalable_targets :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t ->
      DescribeScalableTargetsRequest.t ->
        (DescribeScalableTargetsResponse.t,
          DescribeScalableTargetsResponse.error) Result.t
val describe_scaling_activities :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t ->
      DescribeScalingActivitiesRequest.t ->
        (DescribeScalingActivitiesResponse.t,
          DescribeScalingActivitiesResponse.error) Result.t
val describe_scaling_policies :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t ->
      DescribeScalingPoliciesRequest.t ->
        (DescribeScalingPoliciesResponse.t,
          DescribeScalingPoliciesResponse.error) Result.t
val describe_scheduled_actions :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t ->
      DescribeScheduledActionsRequest.t ->
        (DescribeScheduledActionsResponse.t,
          DescribeScheduledActionsResponse.error) Result.t
val get_predictive_scaling_forecast :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t ->
      GetPredictiveScalingForecastRequest.t ->
        (GetPredictiveScalingForecastResponse.t,
          GetPredictiveScalingForecastResponse.error) Result.t
val list_tags_for_resource :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t ->
      ListTagsForResourceRequest.t ->
        (ListTagsForResourceResponse.t, ListTagsForResourceResponse.error)
          Result.t
val put_scaling_policy :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t ->
      PutScalingPolicyRequest.t ->
        (PutScalingPolicyResponse.t, PutScalingPolicyResponse.error) Result.t
val put_scheduled_action :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t ->
      PutScheduledActionRequest.t ->
        (PutScheduledActionResponse.t, PutScheduledActionResponse.error)
          Result.t
val register_scalable_target :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t ->
      RegisterScalableTargetRequest.t ->
        (RegisterScalableTargetResponse.t,
          RegisterScalableTargetResponse.error) Result.t
val tag_resource :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t ->
      TagResourceRequest.t ->
        (TagResourceResponse.t, TagResourceResponse.error) Result.t
val untag_resource :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t ->
      UntagResourceRequest.t ->
        (UntagResourceResponse.t, UntagResourceResponse.error) Result.t