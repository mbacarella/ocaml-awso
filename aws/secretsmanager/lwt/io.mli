(* generated by: awso-codegen generate-all --botocore-data vendor/botocore/botocore/data -o aws --runtime-dir lib/runtime/awso --cli-dir awso-cli *)
open Awso_secretsmanager.Values
val batch_get_secret_value :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      BatchGetSecretValueRequest.t ->
        (BatchGetSecretValueResponse.t, BatchGetSecretValueResponse.error)
          Result.t Lwt.t
val cancel_rotate_secret :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      CancelRotateSecretRequest.t ->
        (CancelRotateSecretResponse.t, CancelRotateSecretResponse.error)
          Result.t Lwt.t
val create_secret :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      CreateSecretRequest.t ->
        (CreateSecretResponse.t, CreateSecretResponse.error) Result.t Lwt.t
val delete_resource_policy :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DeleteResourcePolicyRequest.t ->
        (DeleteResourcePolicyResponse.t, DeleteResourcePolicyResponse.error)
          Result.t Lwt.t
val delete_secret :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DeleteSecretRequest.t ->
        (DeleteSecretResponse.t, DeleteSecretResponse.error) Result.t Lwt.t
val describe_secret :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DescribeSecretRequest.t ->
        (DescribeSecretResponse.t, DescribeSecretResponse.error) Result.t
          Lwt.t
val get_random_password :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      GetRandomPasswordRequest.t ->
        (GetRandomPasswordResponse.t, GetRandomPasswordResponse.error)
          Result.t Lwt.t
val get_resource_policy :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      GetResourcePolicyRequest.t ->
        (GetResourcePolicyResponse.t, GetResourcePolicyResponse.error)
          Result.t Lwt.t
val get_secret_value :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      GetSecretValueRequest.t ->
        (GetSecretValueResponse.t, GetSecretValueResponse.error) Result.t
          Lwt.t
val list_secret_version_ids :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      ListSecretVersionIdsRequest.t ->
        (ListSecretVersionIdsResponse.t, ListSecretVersionIdsResponse.error)
          Result.t Lwt.t
val list_secrets :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      ListSecretsRequest.t ->
        (ListSecretsResponse.t, ListSecretsResponse.error) Result.t Lwt.t
val put_resource_policy :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      PutResourcePolicyRequest.t ->
        (PutResourcePolicyResponse.t, PutResourcePolicyResponse.error)
          Result.t Lwt.t
val put_secret_value :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      PutSecretValueRequest.t ->
        (PutSecretValueResponse.t, PutSecretValueResponse.error) Result.t
          Lwt.t
val remove_regions_from_replication :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      RemoveRegionsFromReplicationRequest.t ->
        (RemoveRegionsFromReplicationResponse.t,
          RemoveRegionsFromReplicationResponse.error) Result.t Lwt.t
val replicate_secret_to_regions :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      ReplicateSecretToRegionsRequest.t ->
        (ReplicateSecretToRegionsResponse.t,
          ReplicateSecretToRegionsResponse.error) Result.t Lwt.t
val restore_secret :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      RestoreSecretRequest.t ->
        (RestoreSecretResponse.t, RestoreSecretResponse.error) Result.t Lwt.t
val rotate_secret :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      RotateSecretRequest.t ->
        (RotateSecretResponse.t, RotateSecretResponse.error) Result.t Lwt.t
val stop_replication_to_replica :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      StopReplicationToReplicaRequest.t ->
        (StopReplicationToReplicaResponse.t,
          StopReplicationToReplicaResponse.error) Result.t Lwt.t
val tag_resource :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t -> TagResourceRequest.t -> (unit, unit) Result.t Lwt.t
val untag_resource :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t -> UntagResourceRequest.t -> (unit, unit) Result.t Lwt.t
val update_secret :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      UpdateSecretRequest.t ->
        (UpdateSecretResponse.t, UpdateSecretResponse.error) Result.t Lwt.t
val update_secret_version_stage :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      UpdateSecretVersionStageRequest.t ->
        (UpdateSecretVersionStageResponse.t,
          UpdateSecretVersionStageResponse.error) Result.t Lwt.t
val validate_resource_policy :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      ValidateResourcePolicyRequest.t ->
        (ValidateResourcePolicyResponse.t,
          ValidateResourcePolicyResponse.error) Result.t Lwt.t