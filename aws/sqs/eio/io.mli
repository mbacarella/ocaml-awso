(* generated by: awso-codegen generate-all --botocore-data vendor/botocore/botocore/data -o aws --runtime-dir lib/runtime/awso --cli-dir awso-cli *)
open Awso_sqs.Values
val add_permission :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t -> AddPermissionRequest.t -> (unit, unit) Result.t
val cancel_message_move_task :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t ->
      CancelMessageMoveTaskRequest.t ->
        (CancelMessageMoveTaskResult.t, CancelMessageMoveTaskResult.error)
          Result.t
val change_message_visibility :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t ->
      ChangeMessageVisibilityRequest.t -> (unit, unit) Result.t
val change_message_visibility_batch :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t ->
      ChangeMessageVisibilityBatchRequest.t ->
        (ChangeMessageVisibilityBatchResult.t,
          ChangeMessageVisibilityBatchResult.error) Result.t
val create_queue :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t ->
      CreateQueueRequest.t ->
        (CreateQueueResult.t, CreateQueueResult.error) Result.t
val delete_message :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t -> DeleteMessageRequest.t -> (unit, unit) Result.t
val delete_message_batch :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t ->
      DeleteMessageBatchRequest.t ->
        (DeleteMessageBatchResult.t, DeleteMessageBatchResult.error) Result.t
val delete_queue :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t -> DeleteQueueRequest.t -> (unit, unit) Result.t
val get_queue_attributes :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t ->
      GetQueueAttributesRequest.t ->
        (GetQueueAttributesResult.t, GetQueueAttributesResult.error) Result.t
val get_queue_url :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t ->
      GetQueueUrlRequest.t ->
        (GetQueueUrlResult.t, GetQueueUrlResult.error) Result.t
val list_dead_letter_source_queues :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t ->
      ListDeadLetterSourceQueuesRequest.t ->
        (ListDeadLetterSourceQueuesResult.t,
          ListDeadLetterSourceQueuesResult.error) Result.t
val list_message_move_tasks :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t ->
      ListMessageMoveTasksRequest.t ->
        (ListMessageMoveTasksResult.t, ListMessageMoveTasksResult.error)
          Result.t
val list_queue_tags :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t ->
      ListQueueTagsRequest.t ->
        (ListQueueTagsResult.t, ListQueueTagsResult.error) Result.t
val list_queues :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t ->
      ListQueuesRequest.t ->
        (ListQueuesResult.t, ListQueuesResult.error) Result.t
val purge_queue :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t -> PurgeQueueRequest.t -> (unit, unit) Result.t
val receive_message :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t ->
      ReceiveMessageRequest.t ->
        (ReceiveMessageResult.t, ReceiveMessageResult.error) Result.t
val remove_permission :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t -> RemovePermissionRequest.t -> (unit, unit) Result.t
val send_message :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t ->
      SendMessageRequest.t ->
        (SendMessageResult.t, SendMessageResult.error) Result.t
val send_message_batch :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t ->
      SendMessageBatchRequest.t ->
        (SendMessageBatchResult.t, SendMessageBatchResult.error) Result.t
val set_queue_attributes :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t ->
      SetQueueAttributesRequest.t -> (unit, unit) Result.t
val start_message_move_task :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t ->
      StartMessageMoveTaskRequest.t ->
        (StartMessageMoveTaskResult.t, StartMessageMoveTaskResult.error)
          Result.t
val tag_queue :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t -> TagQueueRequest.t -> (unit, unit) Result.t
val untag_queue :
  ?endpoint_url:string ->
    ?cfg:Awso_eio.Cfg.t -> UntagQueueRequest.t -> (unit, unit) Result.t