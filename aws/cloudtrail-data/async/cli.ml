...
let put_audit_events =
  Command.async ~summary:""
    ([%map_open.Command
       ...
       and auditEvents =
         flag "audit-events" (required json_arg) ~doc:"JSON AuditEvents"
       and channelArn =
         flag "channel-arn" (required string) ~doc:"STRING ChannelArn" in
       fun () ->
         call ?endpoint_url ?profile:cli_profile ?region:cli_region
           Io.put_audit_events
           (Values.PutAuditEventsRequest.make ?externalId
              ~auditEvents:(Values.AuditEvents.of_json auditEvents)
              ~channelArn ()) (Some Values.PutAuditEventsResponse.to_json)
           (Some Values.PutAuditEventsResponse.error_to_json)])
let main =
  Command.group
    ~summary:((Awso.Service.to_string Values.service) ^ " commands")
    [("put-audit-events", put_audit_events)]