(* generated by: awso-codegen generate-all --botocore-data vendor/botocore/botocore/data -o aws --runtime-dir lib/runtime/awso --cli-dir awso-cli *)
open Awso_gamesparks.Values
val create_game :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      CreateGameRequest.t ->
        (CreateGameResult.t, CreateGameResult.error) Result.t Lwt.t
val create_snapshot :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      CreateSnapshotRequest.t ->
        (CreateSnapshotResult.t, CreateSnapshotResult.error) Result.t Lwt.t
val create_stage :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      CreateStageRequest.t ->
        (CreateStageResult.t, CreateStageResult.error) Result.t Lwt.t
val delete_game :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DeleteGameRequest.t ->
        (DeleteGameResult.t, DeleteGameResult.error) Result.t Lwt.t
val delete_stage :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DeleteStageRequest.t ->
        (DeleteStageResult.t, DeleteStageResult.error) Result.t Lwt.t
val disconnect_player :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DisconnectPlayerRequest.t ->
        (DisconnectPlayerResult.t, DisconnectPlayerResult.error) Result.t
          Lwt.t
val export_snapshot :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      ExportSnapshotRequest.t ->
        (ExportSnapshotResult.t, ExportSnapshotResult.error) Result.t Lwt.t
val get_extension :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      GetExtensionRequest.t ->
        (GetExtensionResult.t, GetExtensionResult.error) Result.t Lwt.t
val get_extension_version :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      GetExtensionVersionRequest.t ->
        (GetExtensionVersionResult.t, GetExtensionVersionResult.error)
          Result.t Lwt.t
val get_game :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      GetGameRequest.t ->
        (GetGameResult.t, GetGameResult.error) Result.t Lwt.t
val get_game_configuration :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      GetGameConfigurationRequest.t ->
        (GetGameConfigurationResult.t, GetGameConfigurationResult.error)
          Result.t Lwt.t
val get_generated_code_job :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      GetGeneratedCodeJobRequest.t ->
        (GetGeneratedCodeJobResult.t, GetGeneratedCodeJobResult.error)
          Result.t Lwt.t
val get_player_connection_status :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      GetPlayerConnectionStatusRequest.t ->
        (GetPlayerConnectionStatusResult.t,
          GetPlayerConnectionStatusResult.error) Result.t Lwt.t
val get_snapshot :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      GetSnapshotRequest.t ->
        (GetSnapshotResult.t, GetSnapshotResult.error) Result.t Lwt.t
val get_stage :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      GetStageRequest.t ->
        (GetStageResult.t, GetStageResult.error) Result.t Lwt.t
val get_stage_deployment :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      GetStageDeploymentRequest.t ->
        (GetStageDeploymentResult.t, GetStageDeploymentResult.error) Result.t
          Lwt.t
val import_game_configuration :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      ImportGameConfigurationRequest.t ->
        (ImportGameConfigurationResult.t,
          ImportGameConfigurationResult.error) Result.t Lwt.t
val list_extension_versions :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      ListExtensionVersionsRequest.t ->
        (ListExtensionVersionsResult.t, ListExtensionVersionsResult.error)
          Result.t Lwt.t
val list_extensions :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      ListExtensionsRequest.t ->
        (ListExtensionsResult.t, ListExtensionsResult.error) Result.t Lwt.t
val list_games :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      ListGamesRequest.t ->
        (ListGamesResult.t, ListGamesResult.error) Result.t Lwt.t
val list_generated_code_jobs :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      ListGeneratedCodeJobsRequest.t ->
        (ListGeneratedCodeJobsResult.t, ListGeneratedCodeJobsResult.error)
          Result.t Lwt.t
val list_snapshots :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      ListSnapshotsRequest.t ->
        (ListSnapshotsResult.t, ListSnapshotsResult.error) Result.t Lwt.t
val list_stage_deployments :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      ListStageDeploymentsRequest.t ->
        (ListStageDeploymentsResult.t, ListStageDeploymentsResult.error)
          Result.t Lwt.t
val list_stages :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      ListStagesRequest.t ->
        (ListStagesResult.t, ListStagesResult.error) Result.t Lwt.t
val list_tags_for_resource :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      ListTagsForResourceRequest.t ->
        (ListTagsForResourceResult.t, ListTagsForResourceResult.error)
          Result.t Lwt.t
val start_generated_code_job :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      StartGeneratedCodeJobRequest.t ->
        (StartGeneratedCodeJobResult.t, StartGeneratedCodeJobResult.error)
          Result.t Lwt.t
val start_stage_deployment :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      StartStageDeploymentRequest.t ->
        (StartStageDeploymentResult.t, StartStageDeploymentResult.error)
          Result.t Lwt.t
val tag_resource :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      TagResourceRequest.t ->
        (TagResourceResult.t, TagResourceResult.error) Result.t Lwt.t
val untag_resource :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      UntagResourceRequest.t ->
        (UntagResourceResult.t, UntagResourceResult.error) Result.t Lwt.t
val update_game :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      UpdateGameRequest.t ->
        (UpdateGameResult.t, UpdateGameResult.error) Result.t Lwt.t
val update_game_configuration :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      UpdateGameConfigurationRequest.t ->
        (UpdateGameConfigurationResult.t,
          UpdateGameConfigurationResult.error) Result.t Lwt.t
val update_snapshot :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      UpdateSnapshotRequest.t ->
        (UpdateSnapshotResult.t, UpdateSnapshotResult.error) Result.t Lwt.t
val update_stage :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      UpdateStageRequest.t ->
        (UpdateStageResult.t, UpdateStageResult.error) Result.t Lwt.t