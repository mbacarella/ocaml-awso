                ...
                ((match result_to_json with
                  ...
                  | Some to_json ->
                      ((result |> to_json) |> Yojson.Safe.to_string) |>
                        print_endline);
                 return ())))
let assume_role_for_pod_identity =
  Command.async ~summary:""
    ([%map_open.Command
       ...
       and token = flag "token" (required string) ~doc:"STRING JwtToken" in
       fun () ->
         call ?endpoint_url ?profile:cli_profile ?region:cli_region
           Io.assume_role_for_pod_identity
           (Values.AssumeRoleForPodIdentityRequest.make ~clusterName ~token
              ()) (Some Values.AssumeRoleForPodIdentityResponse.to_json)
           (Some Values.AssumeRoleForPodIdentityResponse.error_to_json)])
let main =
  Command.group
    ~summary:((Awso.Service.to_string Values.service) ^ " commands")
    [("assume-role-for-pod-identity", assume_role_for_pod_identity)]