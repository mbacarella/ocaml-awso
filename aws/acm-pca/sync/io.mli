(* generated by: awso-codegen generate-all --botocore-data vendor/botocore/botocore/data -o aws --runtime-dir lib/runtime/awso --cli-dir awso-cli *)
open Awso_acm_pca.Values
val create_certificate_authority :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      CreateCertificateAuthorityRequest.t ->
        (CreateCertificateAuthorityResponse.t,
          CreateCertificateAuthorityResponse.error) Result.t
val create_certificate_authority_audit_report :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      CreateCertificateAuthorityAuditReportRequest.t ->
        (CreateCertificateAuthorityAuditReportResponse.t,
          CreateCertificateAuthorityAuditReportResponse.error) Result.t
val create_permission :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t -> CreatePermissionRequest.t -> (unit, unit) Result.t
val delete_certificate_authority :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DeleteCertificateAuthorityRequest.t -> (unit, unit) Result.t
val delete_permission :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t -> DeletePermissionRequest.t -> (unit, unit) Result.t
val delete_policy :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t -> DeletePolicyRequest.t -> (unit, unit) Result.t
val describe_certificate_authority :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DescribeCertificateAuthorityRequest.t ->
        (DescribeCertificateAuthorityResponse.t,
          DescribeCertificateAuthorityResponse.error) Result.t
val describe_certificate_authority_audit_report :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DescribeCertificateAuthorityAuditReportRequest.t ->
        (DescribeCertificateAuthorityAuditReportResponse.t,
          DescribeCertificateAuthorityAuditReportResponse.error) Result.t
val get_certificate :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      GetCertificateRequest.t ->
        (GetCertificateResponse.t, GetCertificateResponse.error) Result.t
val get_certificate_authority_certificate :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      GetCertificateAuthorityCertificateRequest.t ->
        (GetCertificateAuthorityCertificateResponse.t,
          GetCertificateAuthorityCertificateResponse.error) Result.t
val get_certificate_authority_csr :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      GetCertificateAuthorityCsrRequest.t ->
        (GetCertificateAuthorityCsrResponse.t,
          GetCertificateAuthorityCsrResponse.error) Result.t
val get_policy :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      GetPolicyRequest.t ->
        (GetPolicyResponse.t, GetPolicyResponse.error) Result.t
val import_certificate_authority_certificate :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      ImportCertificateAuthorityCertificateRequest.t -> (unit, unit) Result.t
val issue_certificate :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      IssueCertificateRequest.t ->
        (IssueCertificateResponse.t, IssueCertificateResponse.error) Result.t
val list_certificate_authorities :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      ListCertificateAuthoritiesRequest.t ->
        (ListCertificateAuthoritiesResponse.t,
          ListCertificateAuthoritiesResponse.error) Result.t
val list_permissions :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      ListPermissionsRequest.t ->
        (ListPermissionsResponse.t, ListPermissionsResponse.error) Result.t
val list_tags :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      ListTagsRequest.t ->
        (ListTagsResponse.t, ListTagsResponse.error) Result.t
val put_policy :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t -> PutPolicyRequest.t -> (unit, unit) Result.t
val restore_certificate_authority :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      RestoreCertificateAuthorityRequest.t -> (unit, unit) Result.t
val revoke_certificate :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t -> RevokeCertificateRequest.t -> (unit, unit) Result.t
val tag_certificate_authority :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      TagCertificateAuthorityRequest.t -> (unit, unit) Result.t
val untag_certificate_authority :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      UntagCertificateAuthorityRequest.t -> (unit, unit) Result.t
val update_certificate_authority :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      UpdateCertificateAuthorityRequest.t -> (unit, unit) Result.t