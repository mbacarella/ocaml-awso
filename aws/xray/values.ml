...
module Segment =
  struct
    type nonrec t =
      {
      id: SegmentId.t option ;
      document: SegmentDocument.t option }
...
module SamplingRuleRecord =
  struct
    type nonrec t =
      {
      samplingRule: SamplingRule.t option ;
...
module GetGroupsRequest =
  struct
    type nonrec t = {
      nextToken: GetGroupsNextToken.t option }
    let make ?nextToken = fun () -> { nextToken }
    let to_value x =
      structure_to_value
        [("NextToken",
