  ...
  | DescribeServices ->
      ...
      let headers =
        Awso.Http.Headers.of_list
          ...
          ("X-Amz-Target", "AWSPriceListService.DescribeServices")] in
      Awso.Http.Request.make ~body ~headers (method_of_endpoint endp)
  | GetAttributeValues ->
      let json = GetAttributeValuesRequest.to_json req in
      let body = Yojson.Safe.to_string json in
      let headers =
        Awso.Http.Headers.of_list
          [("Content-Type", "application/x-amz-json-1.1");
...
let of_response (type i) (type o) (type e) (endpoint : (i, o, e) t)
  ...
  | GetAttributeValues ->
      (match resp with
       | Error err ->
           handle_error err (Some GetAttributeValuesResponse.error_of_json)
       | Ok resp ->
           let json = Yojson.Safe.from_string (Awso.Http.Response.body resp) in
           Ok (GetAttributeValuesResponse.of_json json))
  | GetProducts ->
      (match resp with
       | Error err ->
           handle_error err (Some GetProductsResponse.error_of_json)
       | Ok resp ->
           let json = Yojson.Safe.from_string (Awso.Http.Response.body resp) in
