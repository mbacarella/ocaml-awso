(* generated by: awso-codegen generate-all --botocore-data vendor/botocore/botocore/data -o aws --runtime-dir lib/runtime/awso --cli-dir awso-cli *)
open Awso_polly.Values
val delete_lexicon :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DeleteLexiconInput.t ->
        (DeleteLexiconOutput.t, DeleteLexiconOutput.error) Result.t Lwt.t
val describe_voices :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DescribeVoicesInput.t ->
        (DescribeVoicesOutput.t, DescribeVoicesOutput.error) Result.t Lwt.t
val get_lexicon :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      GetLexiconInput.t ->
        (GetLexiconOutput.t, GetLexiconOutput.error) Result.t Lwt.t
val get_speech_synthesis_task :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      GetSpeechSynthesisTaskInput.t ->
        (GetSpeechSynthesisTaskOutput.t, GetSpeechSynthesisTaskOutput.error)
          Result.t Lwt.t
val list_lexicons :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      ListLexiconsInput.t ->
        (ListLexiconsOutput.t, ListLexiconsOutput.error) Result.t Lwt.t
val list_speech_synthesis_tasks :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      ListSpeechSynthesisTasksInput.t ->
        (ListSpeechSynthesisTasksOutput.t,
          ListSpeechSynthesisTasksOutput.error) Result.t Lwt.t
val put_lexicon :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      PutLexiconInput.t ->
        (PutLexiconOutput.t, PutLexiconOutput.error) Result.t Lwt.t
val start_speech_synthesis_stream :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      StartSpeechSynthesisStreamInput.t ->
        (StartSpeechSynthesisStreamOutput.t,
          StartSpeechSynthesisStreamOutput.error) Result.t Lwt.t
val start_speech_synthesis_task :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      StartSpeechSynthesisTaskInput.t ->
        (StartSpeechSynthesisTaskOutput.t,
          StartSpeechSynthesisTaskOutput.error) Result.t Lwt.t
val synthesize_speech :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      SynthesizeSpeechInput.t ->
        (SynthesizeSpeechOutput.t, SynthesizeSpeechOutput.error) Result.t
          Lwt.t