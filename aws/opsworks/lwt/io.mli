(* generated by: awso-codegen generate-all --botocore-data vendor/botocore/botocore/data -o aws --runtime-dir lib/runtime/awso --cli-dir awso-cli *)
open Awso_opsworks.Values
val assign_instance :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t -> AssignInstanceRequest.t -> (unit, unit) Result.t Lwt.t
val assign_volume :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t -> AssignVolumeRequest.t -> (unit, unit) Result.t Lwt.t
val associate_elastic_ip :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      AssociateElasticIpRequest.t -> (unit, unit) Result.t Lwt.t
val attach_elastic_load_balancer :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      AttachElasticLoadBalancerRequest.t -> (unit, unit) Result.t Lwt.t
val clone_stack :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      CloneStackRequest.t ->
        (CloneStackResult.t, CloneStackResult.error) Result.t Lwt.t
val create_app :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      CreateAppRequest.t ->
        (CreateAppResult.t, CreateAppResult.error) Result.t Lwt.t
val create_deployment :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      CreateDeploymentRequest.t ->
        (CreateDeploymentResult.t, CreateDeploymentResult.error) Result.t
          Lwt.t
val create_instance :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      CreateInstanceRequest.t ->
        (CreateInstanceResult.t, CreateInstanceResult.error) Result.t Lwt.t
val create_layer :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      CreateLayerRequest.t ->
        (CreateLayerResult.t, CreateLayerResult.error) Result.t Lwt.t
val create_stack :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      CreateStackRequest.t ->
        (CreateStackResult.t, CreateStackResult.error) Result.t Lwt.t
val create_user_profile :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      CreateUserProfileRequest.t ->
        (CreateUserProfileResult.t, CreateUserProfileResult.error) Result.t
          Lwt.t
val delete_app :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t -> DeleteAppRequest.t -> (unit, unit) Result.t Lwt.t
val delete_instance :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t -> DeleteInstanceRequest.t -> (unit, unit) Result.t Lwt.t
val delete_layer :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t -> DeleteLayerRequest.t -> (unit, unit) Result.t Lwt.t
val delete_stack :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t -> DeleteStackRequest.t -> (unit, unit) Result.t Lwt.t
val delete_user_profile :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DeleteUserProfileRequest.t -> (unit, unit) Result.t Lwt.t
val deregister_ecs_cluster :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DeregisterEcsClusterRequest.t -> (unit, unit) Result.t Lwt.t
val deregister_elastic_ip :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DeregisterElasticIpRequest.t -> (unit, unit) Result.t Lwt.t
val deregister_instance :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DeregisterInstanceRequest.t -> (unit, unit) Result.t Lwt.t
val deregister_rds_db_instance :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DeregisterRdsDbInstanceRequest.t -> (unit, unit) Result.t Lwt.t
val deregister_volume :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DeregisterVolumeRequest.t -> (unit, unit) Result.t Lwt.t
val describe_agent_versions :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DescribeAgentVersionsRequest.t ->
        (DescribeAgentVersionsResult.t, DescribeAgentVersionsResult.error)
          Result.t Lwt.t
val describe_apps :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DescribeAppsRequest.t ->
        (DescribeAppsResult.t, DescribeAppsResult.error) Result.t Lwt.t
val describe_commands :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DescribeCommandsRequest.t ->
        (DescribeCommandsResult.t, DescribeCommandsResult.error) Result.t
          Lwt.t
val describe_deployments :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DescribeDeploymentsRequest.t ->
        (DescribeDeploymentsResult.t, DescribeDeploymentsResult.error)
          Result.t Lwt.t
val describe_ecs_clusters :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DescribeEcsClustersRequest.t ->
        (DescribeEcsClustersResult.t, DescribeEcsClustersResult.error)
          Result.t Lwt.t
val describe_elastic_ips :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DescribeElasticIpsRequest.t ->
        (DescribeElasticIpsResult.t, DescribeElasticIpsResult.error) Result.t
          Lwt.t
val describe_elastic_load_balancers :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DescribeElasticLoadBalancersRequest.t ->
        (DescribeElasticLoadBalancersResult.t,
          DescribeElasticLoadBalancersResult.error) Result.t Lwt.t
val describe_instances :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DescribeInstancesRequest.t ->
        (DescribeInstancesResult.t, DescribeInstancesResult.error) Result.t
          Lwt.t
val describe_layers :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DescribeLayersRequest.t ->
        (DescribeLayersResult.t, DescribeLayersResult.error) Result.t Lwt.t
val describe_load_based_auto_scaling :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DescribeLoadBasedAutoScalingRequest.t ->
        (DescribeLoadBasedAutoScalingResult.t,
          DescribeLoadBasedAutoScalingResult.error) Result.t Lwt.t
val describe_my_user_profile :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      unit ->
        (DescribeMyUserProfileResult.t, DescribeMyUserProfileResult.error)
          Result.t Lwt.t
val describe_operating_systems :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      unit ->
        (DescribeOperatingSystemsResponse.t,
          DescribeOperatingSystemsResponse.error) Result.t Lwt.t
val describe_permissions :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DescribePermissionsRequest.t ->
        (DescribePermissionsResult.t, DescribePermissionsResult.error)
          Result.t Lwt.t
val describe_raid_arrays :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DescribeRaidArraysRequest.t ->
        (DescribeRaidArraysResult.t, DescribeRaidArraysResult.error) Result.t
          Lwt.t
val describe_rds_db_instances :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DescribeRdsDbInstancesRequest.t ->
        (DescribeRdsDbInstancesResult.t, DescribeRdsDbInstancesResult.error)
          Result.t Lwt.t
val describe_service_errors :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DescribeServiceErrorsRequest.t ->
        (DescribeServiceErrorsResult.t, DescribeServiceErrorsResult.error)
          Result.t Lwt.t
val describe_stack_provisioning_parameters :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DescribeStackProvisioningParametersRequest.t ->
        (DescribeStackProvisioningParametersResult.t,
          DescribeStackProvisioningParametersResult.error) Result.t Lwt.t
val describe_stack_summary :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DescribeStackSummaryRequest.t ->
        (DescribeStackSummaryResult.t, DescribeStackSummaryResult.error)
          Result.t Lwt.t
val describe_stacks :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DescribeStacksRequest.t ->
        (DescribeStacksResult.t, DescribeStacksResult.error) Result.t Lwt.t
val describe_time_based_auto_scaling :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DescribeTimeBasedAutoScalingRequest.t ->
        (DescribeTimeBasedAutoScalingResult.t,
          DescribeTimeBasedAutoScalingResult.error) Result.t Lwt.t
val describe_user_profiles :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DescribeUserProfilesRequest.t ->
        (DescribeUserProfilesResult.t, DescribeUserProfilesResult.error)
          Result.t Lwt.t
val describe_volumes :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DescribeVolumesRequest.t ->
        (DescribeVolumesResult.t, DescribeVolumesResult.error) Result.t Lwt.t
val detach_elastic_load_balancer :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DetachElasticLoadBalancerRequest.t -> (unit, unit) Result.t Lwt.t
val disassociate_elastic_ip :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DisassociateElasticIpRequest.t -> (unit, unit) Result.t Lwt.t
val get_hostname_suggestion :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      GetHostnameSuggestionRequest.t ->
        (GetHostnameSuggestionResult.t, GetHostnameSuggestionResult.error)
          Result.t Lwt.t
val grant_access :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      GrantAccessRequest.t ->
        (GrantAccessResult.t, GrantAccessResult.error) Result.t Lwt.t
val list_tags :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      ListTagsRequest.t ->
        (ListTagsResult.t, ListTagsResult.error) Result.t Lwt.t
val reboot_instance :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t -> RebootInstanceRequest.t -> (unit, unit) Result.t Lwt.t
val register_ecs_cluster :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      RegisterEcsClusterRequest.t ->
        (RegisterEcsClusterResult.t, RegisterEcsClusterResult.error) Result.t
          Lwt.t
val register_elastic_ip :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      RegisterElasticIpRequest.t ->
        (RegisterElasticIpResult.t, RegisterElasticIpResult.error) Result.t
          Lwt.t
val register_instance :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      RegisterInstanceRequest.t ->
        (RegisterInstanceResult.t, RegisterInstanceResult.error) Result.t
          Lwt.t
val register_rds_db_instance :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      RegisterRdsDbInstanceRequest.t -> (unit, unit) Result.t Lwt.t
val register_volume :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      RegisterVolumeRequest.t ->
        (RegisterVolumeResult.t, RegisterVolumeResult.error) Result.t Lwt.t
val set_load_based_auto_scaling :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      SetLoadBasedAutoScalingRequest.t -> (unit, unit) Result.t Lwt.t
val set_permission :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t -> SetPermissionRequest.t -> (unit, unit) Result.t Lwt.t
val set_time_based_auto_scaling :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      SetTimeBasedAutoScalingRequest.t -> (unit, unit) Result.t Lwt.t
val start_instance :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t -> StartInstanceRequest.t -> (unit, unit) Result.t Lwt.t
val start_stack :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t -> StartStackRequest.t -> (unit, unit) Result.t Lwt.t
val stop_instance :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t -> StopInstanceRequest.t -> (unit, unit) Result.t Lwt.t
val stop_stack :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t -> StopStackRequest.t -> (unit, unit) Result.t Lwt.t
val tag_resource :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t -> TagResourceRequest.t -> (unit, unit) Result.t Lwt.t
val unassign_instance :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      UnassignInstanceRequest.t -> (unit, unit) Result.t Lwt.t
val unassign_volume :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t -> UnassignVolumeRequest.t -> (unit, unit) Result.t Lwt.t
val untag_resource :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t -> UntagResourceRequest.t -> (unit, unit) Result.t Lwt.t
val update_app :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t -> UpdateAppRequest.t -> (unit, unit) Result.t Lwt.t
val update_elastic_ip :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      UpdateElasticIpRequest.t -> (unit, unit) Result.t Lwt.t
val update_instance :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t -> UpdateInstanceRequest.t -> (unit, unit) Result.t Lwt.t
val update_layer :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t -> UpdateLayerRequest.t -> (unit, unit) Result.t Lwt.t
val update_my_user_profile :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      UpdateMyUserProfileRequest.t -> (unit, unit) Result.t Lwt.t
val update_rds_db_instance :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      UpdateRdsDbInstanceRequest.t -> (unit, unit) Result.t Lwt.t
val update_stack :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t -> UpdateStackRequest.t -> (unit, unit) Result.t Lwt.t
val update_user_profile :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      UpdateUserProfileRequest.t -> (unit, unit) Result.t Lwt.t
val update_volume :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t -> UpdateVolumeRequest.t -> (unit, unit) Result.t Lwt.t