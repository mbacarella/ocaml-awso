(* generated by: awso-codegen generate-all --botocore-data vendor/botocore/botocore/data -o aws --runtime-dir lib/runtime/awso --cli-dir awso-cli *)
open Awso_iot1click_projects.Values
val associate_device_with_placement :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      AssociateDeviceWithPlacementRequest.t ->
        (AssociateDeviceWithPlacementResponse.t,
          AssociateDeviceWithPlacementResponse.error) Result.t Lwt.t
val create_placement :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      CreatePlacementRequest.t ->
        (CreatePlacementResponse.t, CreatePlacementResponse.error) Result.t
          Lwt.t
val create_project :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      CreateProjectRequest.t ->
        (CreateProjectResponse.t, CreateProjectResponse.error) Result.t Lwt.t
val delete_placement :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DeletePlacementRequest.t ->
        (DeletePlacementResponse.t, DeletePlacementResponse.error) Result.t
          Lwt.t
val delete_project :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DeleteProjectRequest.t ->
        (DeleteProjectResponse.t, DeleteProjectResponse.error) Result.t Lwt.t
val describe_placement :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DescribePlacementRequest.t ->
        (DescribePlacementResponse.t, DescribePlacementResponse.error)
          Result.t Lwt.t
val describe_project :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DescribeProjectRequest.t ->
        (DescribeProjectResponse.t, DescribeProjectResponse.error) Result.t
          Lwt.t
val disassociate_device_from_placement :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      DisassociateDeviceFromPlacementRequest.t ->
        (DisassociateDeviceFromPlacementResponse.t,
          DisassociateDeviceFromPlacementResponse.error) Result.t Lwt.t
val get_devices_in_placement :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      GetDevicesInPlacementRequest.t ->
        (GetDevicesInPlacementResponse.t,
          GetDevicesInPlacementResponse.error) Result.t Lwt.t
val list_placements :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      ListPlacementsRequest.t ->
        (ListPlacementsResponse.t, ListPlacementsResponse.error) Result.t
          Lwt.t
val list_projects :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      ListProjectsRequest.t ->
        (ListProjectsResponse.t, ListProjectsResponse.error) Result.t Lwt.t
val list_tags_for_resource :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      ListTagsForResourceRequest.t ->
        (ListTagsForResourceResponse.t, ListTagsForResourceResponse.error)
          Result.t Lwt.t
val tag_resource :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      TagResourceRequest.t ->
        (TagResourceResponse.t, TagResourceResponse.error) Result.t Lwt.t
val untag_resource :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      UntagResourceRequest.t ->
        (UntagResourceResponse.t, UntagResourceResponse.error) Result.t Lwt.t
val update_placement :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      UpdatePlacementRequest.t ->
        (UpdatePlacementResponse.t, UpdatePlacementResponse.error) Result.t
          Lwt.t
val update_project :
  ?endpoint_url:string ->
    ?cfg:Awso.Cfg.t ->
      UpdateProjectRequest.t ->
        (UpdateProjectResponse.t, UpdateProjectResponse.error) Result.t Lwt.t