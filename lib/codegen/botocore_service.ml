...
let fix_shape_name_collisions (service : Botodata.service) =
  ...
  let seen =
    ...
    |> List.fold ~init:String.Set.empty ~f:(fun set (name, _shape) ->
      if begins_lowercase name then set else Set.add set name)
  in
  let fixups = String.Table.create () in
  let collides_when_uppercased name = Set.mem seen (String.capitalize name) in
  ...
  List.iter service.shapes ~f:(fun (name, _shape) ->
    ...
    then (
      ...
      Hashtbl.add_exn fixups ~key:name ~data:fixed_name));
  let maybe_get_fixed_name name =
    Hashtbl.find fixups name |> Option.value ~default:name
  in
