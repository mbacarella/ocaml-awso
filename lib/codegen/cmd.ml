...
module Generate_all = struct
  let header ~argv =
    sprintf
      "(* generated by: awso-codegen generate-all %s *)"
  ...
  let write_dune_file ~argv ~outdir ~data =
    ...
    | Ok { exit_result; stdout; stderr = _ } -> (
      ...
      | Signaled n -> failwithf "dune format-dune-file killed by signal %d" n ()
      | Exited n when n <> 0 -> failwithf "dune format-dune-file exited with code %d" n ()
      | Exited _ ->
  ...
  let main argv =
    ...
    |> List.iter ~f:(fun service ->
      ...
      | Error `Unknown_directory ->
        failwithf "Unknown directory: %s/%s" botocore_data service ()
      | Ok date ->
        let service_json = botocore_data ^/ service ^/ date ^/ "service-2.json" in
        let service_under =
          String.map service ~f:(function
            | '-' -> '_'
            | c -> c)
        ...
        let endpoints_data =
          botoservice |> Service_endpoints.make |> Util.structure_to_string
        in
        write_generated ~argv:argv_strs (base_dir ^/ "endpoints.ml") endpoints_data;
        let num_value_subs = Dune.num_value_submodules service in
        let submodule_files = List.init num_value_subs ~f:(sprintf "values_%d.ml") in
        ...
        let cli_submodule_files = List.init num_cli_subs ~f:(sprintf "cli_%d.ml") in
        let main_cli, sub_clis = Cli.make ~submodules:cli_submodule_files botoservice in
        write_generated
          ~argv:argv_strs
          (async_dir ^/ "cli.ml")
        ...
        if not (Dune.has_addendum ~io_kind:`Async ~service)
        then
          write_generated
            ~argv:argv_strs
        ...
        write_dune_file ~argv:argv_strs ~outdir:lwt_dir ~data:(Dune.make_io `Lwt ~service);
        Io.eval_structure
          ~base_module:(sprintf "Awso_%s" service_under)
          ~io_subsystem:`Lwt
          endpoints
        ...
        write_generated
          ...
          (lwt_dir ^/ "values.ml")
          (sprintf "include Awso_%s.Values" service_under);
        if not (Dune.has_addendum ~io_kind:`Lwt ~service)
        then
          write_generated
            ~argv:argv_strs
            (lwt_dir ^/ sprintf "awso_%s_lwt.ml" service_under)
            "include Io\ninclude Values";
        (* sync io: dune, io.ml, io.mli, values.ml, main module *)
        write_dune_file
          ~argv:argv_strs
          ~outdir:sync_dir
          ~data:(Dune.make_io `Sync ~service);
        Io.eval_structure
          ~base_module:(sprintf "Awso_%s" service_under)
          ~io_subsystem:`Sync
          endpoints
        |> Util.structure_to_string
        |> write_generated ~argv:argv_strs (sync_dir ^/ "io.ml");
        Io.eval_signature
          ~protocol:botoservice.metadata.protocol
          ~base_module:(sprintf "Awso_%s" service_under)
          ~io_subsystem:`Sync
          endpoints
        |> Util.signature_to_string
        |> write_generated ~argv:argv_strs (sync_dir ^/ "io.mli");
        write_generated
          ~argv:argv_strs
          (sync_dir ^/ "values.ml")
          (sprintf "include Awso_%s.Values" service_under);
        if not (Dune.has_addendum ~io_kind:`Sync ~service)
        then
          write_generated
            ~argv:argv_strs
...
let main argv =
  match argv with
  | [||] ->
    eprintf "Usage: awso-codegen {generate-all|build-service-module} [args...]\n";
    exit 1
  | _ -> (
    let sub = argv.(0) in
