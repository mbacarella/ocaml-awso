...
let constants ~awso_service_id service =
  Service_structure.constants_of_service ~awso_service_id service
;;
...
let structure_singleton ~awso_service_id ~loc service shape_modules =
  preamble ~loc ()
  @ constants ~awso_service_id service
  @ errors service
  @ constructors ~loc
  @ shape_modules
;;

let structure_multi ~awso_service_id ~loc index service open_submodules shape_modules =
  match index with
  | 0 ->
    preamble ~loc ()
...
let make ~awso_service_id ~submodules (service : Botodata.service) =
  ...
  | [] ->
    let main_module = structure_singleton ~awso_service_id ~loc service shape_modules in
    let submodules = [] in
  ...
  | submodule_fns ->
    ...
    let submodules =
      List.mapi submodule_fns ~f:(fun i sub_fn ->
        ...
        let struct_ =
          structure_multi ~awso_service_id ~loc i service sub_mods sub_shapes
        in
        sub_fn, struct_)
    in
    main_module, submodules
