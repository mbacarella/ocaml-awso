...
let member_to_arg ~loc { required; arg_name = name; member_shape; shape_name } =
  ...
  let value_expr =
    ...
    | _ -> (
      ...
      let conv = Printf.sprintf "Values.%s.of_json" shape_name |> Ast_convenience.evar in
      match required with
...
let make_structure_for_protocol
  ...
  List.filter_map data ~f:(fun e ->
    ...
    let make_args =
      ...
      | None -> []
      | Some request_shape -> (
        match Map.find shape_map request_shape with
        ...
        | Some (Structure_shape ss) ->
          List.map ss.members ~f:(fun (field_name, member) ->
            ...
            ; member_shape = Map.find_exn shape_map member.shape
            })
    ...
    let call ?endpoint_url ?profile ?region f m result_to_json error_to_json =
      ...
      | Ok result ->
        (match result_to_json with
         ...
         | Some to_json -> result |> to_json |> Yojson.Safe.to_string |> print_endline);
        return ()
    ;;]
;;
