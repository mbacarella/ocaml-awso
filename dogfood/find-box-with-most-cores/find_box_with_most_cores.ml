...
let find_box_with_most_cores ~min_vcpus ~no_spot =
  let%bind rows = fetch_instance_types ~min_vcpus in
  let%bind spot_prices =
    if no_spot
    then return String.Map.empty
    else fetch_spot_prices ~instance_types:(List.map rows ~f:(fun (_, name, _) -> name))
  in
  if not no_spot
  then (
  ...
  let cmd =
    Command.async
      ...
      (let%map_open.Command min_vcpus =
       ...
       and no_spot = flag "--no-spot" no_arg ~doc:" don't fetch current spot prices" in
       fun () -> find_box_with_most_cores ~min_vcpus ~no_spot)
  in
